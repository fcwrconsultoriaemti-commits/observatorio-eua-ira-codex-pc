...
export async function GET(req: NextRequest) {
  try {
    ...
    if (sector && !VALID_SECTORS.includes(sector as any)) {
      return NextResponse.json({ success: false, error: `Setor inválido. Válidos: ${VALID_SECTORS.join(", ")}` }, { status: 400 });
    }

    switch (action) {
      case "profile": {
        if (!sector) return NextResponse.json({ success: false, error: "setor é obrigatório" }, { status: 400 });
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!body.sector) return NextResponse.json({ success: false, error: "setor é obrigatório" }, { status: 400 });
